...
import com.cookandroid.closet.clothModi.ClothModifyEtc
...
class ClothViewEtc : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val position = intent.getIntExtra("positionEtc", 0)
        ...
        binding.clothModify.setOnClickListener {
            val intent = Intent(this, ClothModifyEtc::class.java)
            intent.putExtra("modifyitemEtc", position)
            startActivity(intent)
        }

        binding.clothDelete.setOnClickListener {
            CoroutineScope(Dispatchers.IO).launch {
                val result = db.clothDao().getEtcData()
