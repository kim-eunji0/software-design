...
import com.cookandroid.closet.clothModi.ClothModifyTop
...
class ClothViewTop : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val position = intent.getIntExtra("positionTop", 0)
        ...
        binding.clothModify.setOnClickListener {
            val intent = Intent(this, ClothModifyTop::class.java)
            intent.putExtra("modifyitemTop", position)
            startActivity(intent)
        }

        binding.clothDelete.setOnClickListener {
            CoroutineScope(Dispatchers.IO).launch {
                val result = db.clothDao().getTopData()
