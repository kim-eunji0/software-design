...
import com.cookandroid.closet.clothModi.ClothModifyOuter
...
class ClothViewOuter : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val position = intent.getIntExtra("positionOuter", 0)
        ...
        binding.clothModify.setOnClickListener {
            val intent = Intent(this, ClothModifyOuter::class.java)
            intent.putExtra("modifyitemOuter", position)
            startActivity(intent)
        }

        binding.clothDelete.setOnClickListener {
            CoroutineScope(Dispatchers.IO).launch {
                val result = db.clothDao().getOuterData()
