...
import com.cookandroid.closet.clothModi.ClothModifyCap
...
class ClothViewCap : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val position = intent.getIntExtra("positionCap", 0)
        ...
        binding.clothModify.setOnClickListener {
            val intent = Intent(this, ClothModifyCap::class.java)
            intent.putExtra("modifyitemCap", position)
            startActivity(intent)
        }

        binding.clothDelete.setOnClickListener {
            CoroutineScope(Dispatchers.IO).launch {
                val result = db.clothDao().getCapData()
